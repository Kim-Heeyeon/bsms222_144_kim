...
my_df <- data.frame(state = murders$state,ranks)
my_df
```
<br>
...
murders$state[ind]
my_df <- data.frame(state = murders$state[ind], ind)
my_df
```
<br>
...
```{r}
Ctemp <- (temp -32)*5/9
city_Ctemps <- data.frame(name = city, Ctemp)
city_Ctemps
```
<br>
2. What is the following sum $1+\frac{1}{2^2}+\frac{1}{3^2}+...+\frac{1}{100^2}$
...
```{r}
boxplot(murders$population/10^5~murders$region)
```
